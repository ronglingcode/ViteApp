...
export const createChartWidget = (tabIndex: number, watchlistItem: Models.WatchlistItem, totalCount: number) => {
    ...
    */
    ...
    myWidget.htmlContents.container.addEventListener('mouseleave', function () {
        window.HybridApp.UIState.activeTabIndex = -1;
        const element = document.getElementById("chartContainer" + myWidget.tabIndex);
...
const drawRiskLevelForShort = (symbol: string, widget: Models.ChartWidget) => {
    let topPlan = TradingPlans.getTradingPlans(symbol);
    let defaultRiskLevels = topPlan.analysis.defaultRiskLevels;
