...
export const extractExitPrices = (order: Order) => {
    ...
    if (!order.childOrderStrategies || order.childOrderStrategies.length != 1) {
        return null;
    }
    ...
    orders.forEach(order => {
        ...
        if (!order.childOrderStrategies || order.childOrderStrategies.length != 1) {
            ...
            if (order.childOrderStrategies)
                count = order.childOrderStrategies.length;
            Firestore.logError(`OTO should have only 1 child order, but got ${count} instead`);
            return;
        }
        let firstChild = order.childOrderStrategies[0];
        if (firstChild.orderStrategyType != OrderStrategyType.OCO) {
            Firestore.logError(`OTO child is not OCO, but ${firstChild.orderStrategyType} instead`);
            return;
        }
        let children = extractWorkingChildOrdersFromOCO(firstChild);
        if (children.length == 0) {
        ...
        let exitPair: Models.ExitPair = {
            ...
            parentOrderID: firstChild.orderId ?? "",
        };
