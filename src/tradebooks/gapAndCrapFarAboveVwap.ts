...
export class GapAndCrapFarAboveVwap extends Tradebook {
    public static readonly gapAndCrapFarAboveVwapShort: string = 'G_crap FarAboveVwap';
    private gapAndCrapPlan: TradingPlansModels.GapAndCrapPlan;

    public getID(): string {
        return GapAndCrapFarAboveVwap.gapAndCrapFarAboveVwapShort;
    }
    ...
    constructor(symbol: string, isLong: boolean, plan: TradingPlansModels.GapAndCrapPlan) {
        ...
        if (isLong) {
            throw new Error('GapAndCrapFarAboveVwap tradebook only supports short positions');
        }
        let tradebookName = 'Gap and Crap Far Above VWAP';
        let buttonLabel = 'G_crap FarAboveVwap';
    ...
    triggerEntry(useMarketOrder: boolean, dryRun: boolean, parameters: Models.TradebookEntryParameters): number {
        ...
        let logTagName = '_short_gap_and_crap_far_above_vwap';
    ...
    validateEntry(entryPrice: number, stopOutPrice: number, useMarketOrder: boolean, logTags: Models.LogTags): number {
        ...
        // Open must be far above VWAP (more than 0.5 ATR)
